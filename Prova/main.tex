...
\begin{table}[H]
\centering
\caption{Resultados do cálculo do valor-$p$ para diferentes hipóteses nulas sob o modelo de regressão de Poisson (teste unicaudal à esquerda).}
...
onde $\theta_0 = 1$ e o valor verdadeiro do parâmetro é $\theta = 0{,}20$. 
Em cada replicação, é gerada uma amostra de tamanho $n = 200$ com covariáveis 
...
e o valor-$p$ correspondente é calculado de forma:
...
Quando a hipótese nula se afasta do valor real — como em $H_{0}^{(B)}$ e $H_{0}^{(C)}$ — 
a potência do teste aumenta, os valores-$p$ concentram-se próximos de zero 
e a rejeição de $H_0$ torna-se praticamente certa em cenários de grande discrepância. 
Essa simulação evidencia o comportamento esperado do teste de Wald sob 
o modelo de Poisson e reforça o valor-$p$ como medida de compatibilidade 
entre hipótese estatística e estrutura empírica observada.


\begin{table}[H]
\centering
\caption{Resultados da simulação de Monte Carlo para o teste de Wald em regressão de Poisson. 
Cada linha apresenta os resultados obtidos para uma hipótese nula distinta, com $M = 10{,}000$ replicações.}
\label{tab:montecarlo}
\begin{tabular}{lccccccc}
\toprule
\textbf{Cenário} & \textbf{Hipótese nula} & \textbf{\% de rejeição} & \textbf{Média($p$)} & \textbf{Mediana($p$)} & \textbf{$p_{0{,}05}$} & \textbf{$p_{0{,}95}$} & \textbf{$\bar{\hat{\theta}}$ / $\theta$} \\
\midrule
Cenário A 
& $H_{0}^{(A)} = 0{,}20$ 
& 0{,}047700 
...
# -------- Formatação (sem notação científica e 6 casas decimais) --------
options(scipen = 999)
