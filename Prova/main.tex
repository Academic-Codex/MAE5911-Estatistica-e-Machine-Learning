...
\end{lstlisting}
    ...
    \begin{answer}[Questão 03]
        \section{Regressão Binomial Robusta: Formulação e Evidências}

\subsection{Modelo e Construção do Experimento}

Considerou-se o modelo binomial
\[
...
A variável explicativa foi gerada como $X_i \sim U(-2,2)$ e as respostas $Y_i$ a partir do modelo binomial acima.  
...
\end{cases}
\]
Esse procedimento gera simultaneamente \textit{outliers de alavancagem} (no eixo $x$) e \textit{outliers de resposta} (no eixo $y$), de modo análogo à estratégia demonstrada em sala.

\subsection{Função de Estimação Robusta}

O estimador clássico de máxima verossimilhança (MLE) foi obtido pela regressão logística usual:
\[
\hat{\theta}_{\text{MLE}} = 
\arg\max_{\theta} 
\sum_{i=1}^{n} 
...
Essa construção faz com que observações cujos resíduos ultrapassam o limite $c = p\,\mathrm{IQR}(r)$ recebam peso reduzido, atenuando sua influência no ajuste.

\subsection{Simulação e Resultados}

Foram realizadas $R = 500$ repetições do experimento (Monte Carlo), calculando-se para cada método a média, o viés e o erro quadrático médio (RMSE) das estimativas.  
...
\subsection{Evidências Gráficas}

A Figura~\ref{fig:curvas_binomial_robusta} mostra as curvas ajustadas pelo MLE e pelo método robusto.  
Os pontos destacados em vermelho correspondem às observações atenuadas pelo corte baseado no IQR.
...
A Figura~\ref{fig:hist_binomial_robusta} apresenta as distribuições empíricas (histogramas) das estimativas sob $R=500$ repetições.  
As curvas vermelhas representam a densidade normal ajustada a cada conjunto de estimativas, e as linhas tracejadas indicam os valores verdadeiros dos parâmetros.
...
Os resultados mostram que a contaminação de apenas $10\%$ das observações foi suficiente para produzir um viés expressivo no estimador clássico de máxima verossimilhança, especialmente no coeficiente $\beta_1$ (viés médio de $-0.355$).  
