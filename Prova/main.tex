...
\]
isto é, a probabilidade de se observar uma estatística tão ou mais extrema quanto a obtida, assumindo $H_0$ verdadeira.
...
um deslize epistemológico em que se tenta extrair informação empírica de uma construção que em sua definição é epistêmica e condicional.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Monte Carlo
O experimento de Monte Carlo apresentado na Listing~\ref{lst:poisson-monte-carlo} tem como objetivo avaliar, de forma empírica, 
...
— como em $H_{0}^{(B)}$ e $H_{0}^{(C)}$ — os valores presentes na porção unicaudal à esquerda 
se tornam mais expressivos, e os valores-$p$ tornam-se progressivamente menores. 
...
## Design: x é gerado 1 vez e mantido fixo
x = runif(n, 0, 2)
...
Seja $K\subset\mathbb{R}^n$ compacto e $\sigma:\mathbb{R}\to\mathbb{R}$ localmente limitada. As combinações finitas
\[
x\longmapsto \sum_{j=1}^N a_j\,\sigma(w_j^\top x+b_j)
\]
são densas em $C(K)$ se, e somente se, $\sigma$ não é um polinômio. Consequentemente, para qualquer $f\in C(K)$ e $\varepsilon>0$,
existem $N$, pesos e vieses tais que
\[
\sup_{x\in K}\Bigl\|f(x)-\sum_{j=1}^N a_j\,\sigma(w_j^\top x+b_j)\Bigr\|<\varepsilon.
\]
O resultado é demonstrado para funções escalares, mas vale consequentemente para saídas vetoriais, isto é, para redes neurais com vários outputs, aproximando cada componente de $f:K\to\mathbb{R}^m$.

Este resultado generaliza os de Funahashi (1989) e Cybenko (1989), estendendo ao teorema da universalidade funções 
de ativação não sigmoidais, como a ReLU. A prova mostra que a propriedade essencial não depende da forma específica da ativação, 
mas apenas da condição fundamental de que ela não seja um polinômio.
...
\end{cases}
\]
Gerando \textit{outliers} com desvio em relação ao eixo $x$, de resposta, e \textit{outliers} com desvio em relação ao eixo $y$, de alavancagem.
\subsubsection*{Função de Estimação Robusta}

O estimador de máxima verossimilhança (MLE) é obtido pela 
minimização do negativo da log-verossimilhança binomial, 
cuja parametrização em termos do logito conduz ao modelo de regressão logística.  
Nesse caso, os parâmetros $\theta = (\beta_0, \beta_1)$ são estimados como
\[
\hat{\theta}_{\text{MLE}} = 
\arg\min_{\theta}
\Big\{
...
\Big\},
\]
em que $\mu_{\theta}(x_i) = \frac{1}{1 + e^{-(\beta_0 + \beta_1 x_i)}}$ 
representa a probabilidade de sucesso segundo o modelo logístico.
...
A implementação da regressão binomial robusta é apresentada no \textit{Listing}~\ref{lst:binomial-robusta} e define uma função de estimação ponderada baseada nos resíduos 
\(
r_i = y_i - m\mu_\theta(x_i),
\)
com pesos adaptativos
\(
w_i = \exp\!\big(-(|r_i|-c)_+\big), \quad c = p \cdot \text{IQR}(r),
\)
em que o ponto de corte \(c\) é proporcional à dispersão interquartil dos resíduos, conforme já apresentada. 

A Figura~\ref{fig:curvas_binomial_robusta} ilustra o comportamento do ajuste em um único experimento, 
mostrando as curvas estimadas pelo MLE e pelo método robusto em comparação com a curva verdadeira dos dados. 
Os pontos destacados em vermelho correspondem às observações atenuadas pela ponderação robusta.
...
O \textit{Listing}~\ref{lst:binomial-monte-carlo} apresenta o procedimento de simulação de Monte Carlo correspondente, 
no qual foram realizadas \(R = 500\) repetições para comparar o desempenho do estimador de máxima verossimilhança (MLE) com o estimador robusto proposto. 
Para cada método, foram calculadas a média, o viés e o erro quadrático médio (RMSE) das estimativas, cujos resultados são apresentados na Tabela~\ref{tab:binomial_robusta} abaixo.
...
Os resultados mostram que o MLE apresenta subestimação do parâmetro verdadeiro, com viés negativo em $\beta_1$
e maior erro quadrático médio. O estimador robusto, por sua vez, reduziu o viés de $\beta_1$ em aproximadamente \textbf{77\%} 
e o RMSE em cerca de \textbf{72\%}.
Para $\beta_0$, observou-se uma redução de \textbf{74\%} no viés e de \textbf{22\%} no RMSE.

A Figura~\ref{fig:hist_binomial_robusta} apresenta as distribuições empíricas das estimativas obtidas nas simulações de Monte Carlo. 
As curvas vermelhas correspondem às densidades normais ajustadas com base na média e no desvio-padrão empíricos, 
evidenciando a característica de normalidade assintótica esperada para os estimadores. 
Observa-se que o estimador robusto manteve as distribuições centradas próximas aos valores verdadeiros, 
apresentando \textbf{menor viés e menor erro quadrático médio (RMSE)} em comparação ao MLE.
...
— um viés negativo mais evidente em $\beta_1$. 
...
As ponderações e atenuações com base no desvio em relação ao intervalo interquatil dos resíduos reduzem 
com eficiencia a influência de observações com maiores desvios de resposta e de alavancagem.

O estimador robusto, portanto, reduziu simultaneamente o viés e a variância das estimativas, levando à minimização do erro quadrático médio (EQM).
 \end{answer}
