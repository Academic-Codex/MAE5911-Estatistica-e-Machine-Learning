...
\midrule
\texttt{block\_size}     & 16 & contexto máximo (tokens anteriores). \\
...
O tamanho de contexto (\texttt{block\_size = 16}) limita o alcance da atenção, tornando o treinamento mais leve e viável em hardware pessoal. 
...
Durante o treinamento, foi utilizada a função de perda \emph{cross-entropy},
...
\texttt{1.90} para o conjunto de treino e \texttt{1.96} para o conjunto de teste. 
...
Em síntese, este experimento demonstra a implementação completa de um modelo 
Transformer treinado do zero, com pipeline de preparação do corpus, definição 
dos hiperparâmetros, arquitetura interna e laço de treinamento. Mesmo sendo um 
modelo compacto, ele é capaz de aprender padrões linguísticos ricos graças à 
estrutura sofisticada do mecanismo de atenção.

Esperamos observar que, após certo número de épocas, o modelo consiga gerar 
texto coerente em estilo Shakespeareano, preservando elementos estruturais 
como métrica aproximada, vocabulário renascentista e padrões sintáticos 
recorrentes.
    \end{answer}
