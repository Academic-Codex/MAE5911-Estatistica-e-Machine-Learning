...
\subsection*{Cálculo do EMV de \(\theta\) ( \(\hat\theta_{MV}\)) para a distribuição Beta}


%=========================================================
% Função de verossimilhança — Questão 04 (Beta(θ,1))
%=========================================================

\subsection*{Função de verossimilhança}

Seja $(z_1,\dots,z_n)$ uma amostra aleatória de \quad $Z \sim f_\theta, \qquad \theta \in (0,\infty)$, \quad com função densidade de probabilidade:
\[
  f_\theta(z_i) = \theta\, z_i^{\theta-1}, \qquad 0<z_i<1,
\]
e $f_\theta(z_i)=0$ caso contrário.

Como $z_1,\dots,z_n$ são independentes, a função de verossimilhança é o produto das densidades individuais:
\[
  \ell(\theta;z_1,\dots,z_n)
  = \prod_{i=1}^n f_\theta(z_i)
  = \prod_{i=1}^n \theta\,z_i^{\theta-1}.
\]

Aplicando a propriedade distributiva do produto, obtemos:
\[
  \ell(\theta;z_1,\dots,z_n)
  = \theta^n \prod_{i=1}^n z_i^{\theta-1}.
\]

Seja, e utilizando a identidade $a^{\,b} = e^{\,b \log a}$:
\[
  T = \sum_{i=1}^n \log z_i \qquad
  e
  \qquad
  \displaystyle \prod_{i=1}^n z_i^{\theta-1}
  = \exp\!\left((\theta-1)\sum_{i=1}^n \log z_i\right)
  = \exp\!\bigl((\theta-1)T\bigr),
\]

Podemos escrever:
\[
  \ell(\theta,T)
  = \theta^n \exp\!\bigl((\theta-1)T\bigr).
\]

Para simplificar os cálculos, trabalhamos com a função logaritmo da verossimilhança \(\mathcal{L}(\theta;T)\):
\[
  \mathcal{L}(\theta;T)
  = \log\ell(\theta;T)
  = \log\bigl(\theta^n\bigr)
  + \log\!\left[\exp\!\bigl((\theta-1)T\bigr)\right]
  = n\log\theta + (\theta-1)T.
\]

%--------------------------------------------------------
% Derivada em relação a θ
%--------------------------------------------------------

Calculando a derivada de $\mathcal{L}(\theta,T)$ em relação a $\theta$:
\[
  \frac{\partial \mathcal{L}(\theta,T)}{\partial\theta}
  =
  \frac{\partial}{\partial\theta}
  \bigl[n\log\theta + (\theta-1)T\bigr]
  =
  \frac{n}{\theta} + T.
\]

Seja a condição de máximo onde está definido $\widehat\theta_{MV}$:
\[
  \frac{\partial \mathcal{L}(\theta,T)}{\partial\theta} = 0
  \;\Longleftrightarrow\;
  \frac{n}{\widehat\theta_{MV}} + T = 0
  \;\Longleftrightarrow\;
  \frac{n}{\widehat\theta_{MV}} = -T
\]

Isolando $\widehat\theta_{MV}$:
\[
  \boxed{
    \widehat\theta_{MV}
    = -\,\frac{n}{T}
...
\[
  \boxed{
    \widehat\theta_{MV}
    = -\,\frac{n}{\displaystyle\sum_{i=1}^n \log z_i}
  }.
\]
