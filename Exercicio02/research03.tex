...
% Para simplificar os cálculos, trabalhamos com a função
...
Agora, igualando a zero para encontrar o ponto crítico:



Como \( S = \sum_{i=1}^n z_i \), isso é exatamente

\[
    \widehat{\mu}_{MV} = \bar Z = \frac{1}{n}\sum_{i=1}^n z_i.
\]
%%
...
\[
    ...
    \sum_{i=1}^n z_i = n\mu
    \;\Longleftrightarrow\;
    \widehat\mu_{MV} = \overline Z
    = \frac{1}{n}\sum_{i=1}^n z_i.
\]

%--------------------------- Derivada em relação a \sigma^2

Agora derivamos em relação a $\sigma^2$.
É conveniente reescrever $\ell$ como função de $\sigma^2$:
\[
    \ell(\mu,\sigma^2)
    = -\frac{n}{2}\log(2\pi)
    -\frac{n}{2}\log(\sigma^2)
...
\[
    ...
    + \frac{1}{2}\,\frac{\sum_{i=1}^n (z_i-\mu)^2}{(\sigma^2)^2}.
\]
...
\[
    -n\sigma^2 + \sum_{i=1}^n (z_i-\mu)^2 = 0
    \;\Longleftrightarrow\;
    \sigma^2
    = \frac{1}{n}\sum_{i=1}^n (z_i-\mu)^2.
\]

Substituindo $\mu$ pelo seu EMV $\widehat\mu_{MV}=\overline Z$, obtemos
o EMV de $\sigma^2$:
\[
    \widehat\sigma^2_{MV}
...
Portanto, o **vetor de estimadores de máxima verossimilhança** é
\[
    \boxed{
    \widehat\mu_{MV} = \overline Z,
    \qquad
...
Pela **invariância do EMV**, para qualquer função
$g(\mu,\sigma^2)$ o EMV de $g$ é dado por
\[
