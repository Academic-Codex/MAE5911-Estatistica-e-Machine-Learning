...
Para a Normal $N(\mu,\sigma^2)$, a esperança é
\[
  E_\theta(Z) = \mu.
\]
Logo,
...
Para uma Normal $N(\mu,\sigma^2)$, podemos padronizar:
\[
  P_\theta(Z<2)
  = P\!\left(
  \frac{Z-\mu}{\sigma}
  ...
  \right),
\]
onde $\Phi(\cdot)$ é a função de distribuição da Normal padrão $N(0,1)$
e $\sigma=\sqrt{\sigma^2}$.
...
Pela invariância do EMV, o estimador de máxima verossimilhança de
$g(\theta)$ é
\[
  \widehat g_{MV}
